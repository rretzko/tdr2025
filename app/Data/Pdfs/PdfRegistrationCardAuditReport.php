...
//        $this->test($candidateIds);
        return DB::table('candidates')
            ...
            ->leftJoin('phone_numbers AS studentPhone', function($join) {
                $join->on('users.id', '=', 'studentPhone.user_id')
    ...
    private function test(array $candidateIds): void
    {
        $query = DB::table('candidates')
            ...
            ->leftJoin('phone_numbers AS studentPhone', 'users.id', '=', 'studentPhone.user_id')
            ->whereIn('candidates.id', $candidateIds)
//            ->where('studentPhone.phone_type', 'mobile')
            ->where('teacherPhone.phone_type', 'mobile')
            ->where('schools.id', 4721)
            ->select(
                'candidates.ref AS id',
                DB::raw("CONCAT(users.last_name, ', ', users.first_name, ' ', users.middle_name) AS alphaName"),
                'users.email AS email',
