...
<div>
    ...
    <div class="hidden sm:block ">
        <div class="border border-gray-200 border-b-transparent shadow-lg">
            <nav class="flex row justify-start mx-8 space-x-4 " aria-label="Tabs">

                @foreach($tabs AS $tab)
                    <a href="{{ $domain . $tab }}" wire:navigate>
                        <button @class([
                            'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700 w-full border-b-2 py-4 px-1 text-center text-sm font-medium',
                    ...
                    {{--                        <button type="button" wire:click="$set('selectedTab', '{{ $tab }}')" wire:key="{{ $tab }}"--}}
                    {{--                            @class([--}}
